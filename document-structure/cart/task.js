...
function addingProductToCart(idProduct, indexProduct) {
  const imageProduct = document.querySelectorAll('.product__image')[indexProduct].getAttribute("src");
  const quantityProduct = document.querySelectorAll('.product__quantity-value')[indexProduct];

  const arrOfProductsInCart = [...document.querySelectorAll('.cart__product')];
  const indexProductInCart = arrOfProductsInCart.findIndex(inCart => inCart.dataset.id === idProduct); 

  if (indexProductInCart > -1) { 
    const quantityProductInCart = document.querySelectorAll('.cart__product-count')[indexProductInCart];
